    ...
    return (
      <div className="min-h-[80vh] bg-bg flex items-center justify-center px-6">
        <div className="text-center max-w-md">
          <div className="w-16 h-16 bg-brand/5 text-brand rounded-2xl flex items-center justify-center mx-auto mb-6 border border-brand/10">
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <h1 className="font-display text-2xl font-black text-ink uppercase tracking-tight mb-3">
          ...
          <Link
            href="/aanbod/"
            className="inline-flex items-center gap-2 px-8 py-3.5 bg-brand text-white font-bold uppercase tracking-wider rounded-full hover:bg-brand-deep transition-all duration-300 text-xs shadow-md shadow-brand/15"
          >
  ...
  return (
    <div className="bg-bg min-h-screen py-6 lg:py-16 px-4 sm:px-6 lg:px-10 pb-28 md:pb-16">
      <div className="max-w-[1200px] mx-auto">
        ...
        <Reveal>
          <div className="flex items-center gap-2 text-[11px] lg:text-xs text-muted/70 font-semibold uppercase tracking-wider mb-6 lg:mb-10">
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-16 items-start mb-12 lg:mb-20">
          ...
          <div className="lg:col-span-6">
            <Reveal variant="scale" className="relative w-full aspect-square sm:aspect-[4/3] lg:aspect-square bg-white border border-hairline/45 rounded-3xl overflow-hidden flex items-center justify-center p-6 sm:p-10 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
            ...
            {product.images && product.images.length > 0 && (
              <Reveal delay={100} className="flex flex-wrap gap-2.5 mt-4 justify-center lg:justify-start">
                {[product.image, ...product.images].map((img, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-14 h-14 sm:w-16 sm:h-16 bg-white border rounded-2xl overflow-hidden p-2 transition-all duration-300 ${
                        isSelected ? "border-brand ring-2 ring-brand/20 scale-95" : "border-hairline hover:border-brand/40 hover:scale-102"
                      }`}
                    >
          ...
          <div className="lg:col-span-6 flex flex-col pt-2 lg:pt-4">
            ...
            {/* Category / Brand Row */}
            <Reveal>
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-hairline/35">
            ...
            {/* Title */}
            <Reveal delay={60}>
              <h1 className="font-display font-black text-2xl sm:text-3xl lg:text-4xl text-ink uppercase tracking-tight leading-[1.1] mb-4">
                {product.name}
              </h1>
            </Reveal>
            ...
            <Reveal delay={180} className="hidden md:block mb-8">
              <Link
                href={quoteUrl}
                className="inline-flex items-center justify-center gap-2.5 px-9 py-4 bg-brand text-white text-xs font-black uppercase tracking-[0.2em] rounded-full hover:bg-brand-deep hover:shadow-lg hover:shadow-brand/20 active:translate-y-px transition-all duration-300 shadow-md shadow-brand/10"
              >
            ...
            <Reveal delay={240} className="mb-8">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-muted/60 mb-2">
                {language === "nl" ? "Productomschrijving" : "Product Description"}
              </h2>
              <p className="text-sm sm:text-base text-copy leading-relaxed font-medium">
                {product.description}
              </p>
            </Reveal>

            {/* Trust Points (Compact Icons Grid) */}
            <Reveal delay={300} className="border-t border-hairline/35 pt-6 lg:pt-8">
              <div className="grid sm:grid-cols-2 gap-y-3.5 gap-x-4">
                {[
                  { 
                  ...
                  <div key={idx} className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-[11px] font-black text-ink uppercase tracking-tight leading-tight">
                        {item.label}
                      </h4>
                      <p className="text-[10px] text-muted leading-tight mt-0.5 font-medium">
                        {item.desc}
                      </p>
                    </div>
                  </div>
        ...
        {product.techSpecs && product.techSpecs.length > 0 && (
          <div className="border-t border-hairline/35 pt-12 lg:pt-16">
            <Reveal>
              <h2 className="font-display font-black text-xl lg:text-2xl text-ink uppercase tracking-tight mb-6 lg:mb-8">
                {language === "nl" ? "Technische Eigenschappen" : "Technical Specifications"}
              </h2>
            </Reveal>

            <Reveal delay={80} className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
              {product.techSpecs.map((spec, idx) => (
                <div key={idx} className="bg-white border border-hairline/45 rounded-2xl p-4.5 hover:border-brand/40 transition-colors duration-300 flex items-center gap-3">
                  <div className="w-7 h-7 rounded-lg bg-concrete border border-hairline/30 flex items-center justify-center shrink-0">
                  ...
                  <span className="text-[11px] lg:text-xs font-bold text-ink uppercase tracking-tight leading-snug">
                    {spec}
                  </span>
                </div>
              ))}
      ...
      <div className="fixed bottom-[var(--bottom-nav-height)] md:bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-hairline/45 p-4 flex items-center justify-between shadow-[0_-8px_30px_rgba(0,0,0,0.05)] md:hidden safe-bottom">
        <div className="flex flex-col min-w-0 pr-3">
          <span className="text-[9px] font-black text-muted uppercase tracking-widest truncate max-w-[140px]">
            {product.name}
          </span>
          <span className="text-sm font-black text-brand tracking-tight leading-none mt-1">
            {product.price || (language === "nl" ? "Prijs op aanvraag" : "Price on request")}
          </span>
        </div>
        <Link
          href={quoteUrl}
          className="px-6 py-3.5 bg-brand text-white text-[11px] font-black uppercase tracking-wider rounded-full hover:bg-brand-deep active:scale-98 transition-all shrink-0"
        >
