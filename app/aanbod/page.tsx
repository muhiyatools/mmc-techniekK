...
function PriceRangeSlider({ value, onChange }: { value: [number, number]; onChange: (v: [number, number]) => void }) {
  ...
  const el = (p: string) => p; // suppress unused
  el("ref"); // avoid unused warning
        ...
        <div className="max-w-[1280px] mx-auto px-6 lg:px-10 pt-8 pb-24 lg:pb-32">
          ...
          <div className="lg:hidden mb-5 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 flex-wrap min-w-0">
              {activeService && (
                <span className="flex items-center gap-1.5 text-xs font-bold text-brand bg-brand/8 border border-brand/20 px-3 py-1 rounded-full">
                  {activeService.title}
                  <button
                    onClick={() => { setActiveSlug(null); handleResetFilters(); }}
              ...
              {selectedBrands.map((b) => (
                <span key={`mobile-chip-${b}`} className="flex items-center gap-1.5 text-xs font-bold text-ink bg-concrete border border-hairline px-3 py-1 rounded-full">
                  {b}
                  <button onClick={() => setSelectedBrands((prev) => prev.filter((x) => x !== b))} className="ml-0.5 text-muted hover:text-ink transition-colors" aria-label={`Verwijder ${b} filter`}>
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </span>
              ))}
              {(priceRange[0] > SLIDER_MIN || priceRange[1] < SLIDER_MAX) && activeService && (
                <span className="flex items-center gap-1.5 text-xs font-bold text-ink bg-concrete border border-hairline px-3 py-1 rounded-full">
                  ...
                  <button onClick={() => setPriceRange([SLIDER_MIN, SLIDER_MAX])} className="ml-0.5 text-muted hover:text-ink transition-colors" aria-label="Verwijder prijsfilter">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </span>
              )}
              {!activeService && <span className="text-xs text-muted">Kies een categorie</span>}
            </div>
            <button
            ...
            >
              ...
              Filter
              {(activeSlug || selectedBrands.length > 0 || priceRange[0] > SLIDER_MIN || priceRange[1] < SLIDER_MAX) && <span className="w-2 h-2 rounded-full bg-brand shrink-0" />}
            </button>
          </div>
          ...
          <div className="flex gap-10 lg:gap-12">
            ...
            <div className="flex-1 min-w-0" id="aanbod-content">
              {!activeSlug && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {services.map((service, i) => (
                    <Reveal key={`cat-${service.slug}`} delay={i * 40}>
                      <button
                      ...
                      >
                        <div className="relative aspect-video overflow-hidden">
                          <Image src={service.image} alt={service.title} fill className="object-cover transition-transform duration-700 ease-out group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 640px" />
              ...
              {activeSlug && activeService && (
                <>
                  {/* Editorial dark brief */}
                  <Reveal>
                    <div className="relative overflow-hidden rounded-2xl mb-7">
                      <div className="absolute inset-0">
                        <Image src={activeService.image} alt={activeService.title} fill className="object-cover opacity-20" sizes="1280px" />
                      ...
                      <div className="relative z-10 p-6 lg:p-10">
                        <div className="flex flex-col sm:flex-row sm:items-end gap-5 sm:gap-8">
                          <div className="flex-1 min-w-0">
                            <p className="text-[0.6rem] font-bold uppercase tracking-[0.2em] text-brand mb-2">{activeService.title}</p>
                            <h2 className="font-display font-extrabold text-2xl lg:text-3xl text-white mb-3 leading-tight tracking-tight">{activeService.caption}</h2>
                            <p className="text-sm text-white/70 leading-relaxed max-w-2xl">{activeService.description}</p>
                          </div>
                          <Link href={`/contact?service=${activeService.slug}`} className="shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-brand text-white text-xs font-bold rounded-full hover:bg-brand-deep transition-colors duration-200 shadow-sm shadow-brand/20 uppercase tracking-wide whitespace-nowrap">
                            Offerte aanvragen
                    ...
                    <>
                        <div className="flex items-center justify-between mb-5">
                          <p className="text-sm text-muted">
                            <span className="font-bold text-ink">{filteredProducts.length}</span> van <span className="font-bold text-ink">{activeService.products.length}</span> producten
                            {(selectedBrands.length > 0 || priceRange[0] > SLIDER_MIN || priceRange[1] < SLIDER_MAX) && (
                              <button onClick={handleResetFilters} className="ml-2 text-brand font-semibold hover:text-brand-deep transition-colors">(filter wissen)</button>
                            )}
                          </p>
                      ...
                      {filteredProducts.length > 0 ? (
                            ...
                            <Reveal key={`${activeService.slug}-${product.name}-${i}`} delay={i * 50}>
                              <div className="group bg-surface border border-hairline hover:border-brand/30 hover:shadow-lg hover:shadow-brand/5 transition-all duration-300 overflow-hidden flex flex-col h-full rounded-2xl">
                                ...
                                <div className="p-5 flex flex-col flex-1">
                                  ...
                                  <h4 className="text-sm font-bold text-ink leading-snug mb-2 group-hover:text-brand transition-colors duration-200">{product.name}</h4>
                                  ...
                                  <ul className="space-y-1 mb-4">
                                    {product.techSpecs.slice(0, 3).map((spec, idx) => (
                                      <li key={`${product.name}-spec-${idx}`} className="flex items-start gap-2 text-xs text-copy">
                                        <svg className="w-3 h-3 text-brand shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                                        {spec}
                                      </li>
                                    ))}
                                  </ul>
                                  <div className="mt-auto pt-4 border-t border-hairline">
                                    <div className="flex items-end justify-between gap-3">
                                      {product.price ? (
                                        <div>
                                          <p className="text-[0.6rem] font-bold uppercase tracking-[0.15em] text-muted mb-0.5">Incl. installatie</p>
                                          <span className="text-base font-extrabold text-ink tabular-nums">{product.price}</span>
                                        </div>
                                      ) : (
                                        <span className="text-xs text-muted italic">Prijs op aanvraag</span>
                                      )}
                                      <Link href={`/contact?service=${activeService.slug}&product=${encodeURIComponent(product.name)}`} className="shrink-0 px-4 py-2.5 bg-brand text-white text-xs font-bold rounded-full hover:bg-brand-deep transition-colors duration-200 shadow-sm shadow-brand/20">
                                        Offerte aanvragen
                                      </Link>
                                    </div>
                                  </div>
                        ...
                        <div className="py-16 text-center border border-hairline bg-concrete rounded-2xl">
                          <svg className="w-8 h-8 text-muted mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
                          <p className="text-sm text-muted mb-3">Geen producten gevonden voor de geselecteerde filters.</p>
                          <button onClick={handleResetFilters} className="text-sm font-bold text-brand hover:text-brand-deep transition-colors">Filter wissen</button>
                        </div>
                      )}
                    </>
                  ) : (
                    <Reveal>
                      <div className="bg-concrete border border-hairline p-8 lg:p-12 flex flex-col sm:flex-row items-start sm:items-center gap-6 rounded-2xl">
                        ...
                        <div className="flex-1">
                          ...
                          <h3 className="text-xl font-bold text-ink mb-2">Deze dienst wordt volledig op maat samengesteld</h3>
                          <p className="text-sm text-muted leading-relaxed">Neem contact op voor een vrijblijvend adviesgesprek. Wij brengen uw situatie in kaart en stellen een offerte op maat op.</p>
                        </div>
                        <Link href={`/contact?service=${activeService.slug}`} className="shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-brand text-white text-sm font-bold rounded-full hover:bg-brand-deep transition-colors duration-200 shadow-sm shadow-brand/20 uppercase tracking-wide">
                          Offerte aanvragen
