...
function ContactForm() {
  ...
  return (
    <>
      ...
      <div
        className="fixed left-0 right-0 md:hidden z-[49] flex gap-3 bg-surface border-t border-hairline px-4 py-3"
        style={{ bottom: "calc(var(--bottom-nav-height) + var(--safe-bottom))" }}
      >
        <a
          href={`tel:${WA_PHONE}`}
          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-hairline text-ink font-semibold text-sm active:bg-concrete transition-colors"
        >
        ...
        <a
          ...
          className="flex-[1.5] flex items-center justify-center gap-2 py-3 rounded-xl bg-[#25D366] text-white font-bold text-sm active:opacity-90 transition-opacity"
        >
